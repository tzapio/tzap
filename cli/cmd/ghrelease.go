...
The release is then created on GitHub using the title and notes generated by ChatGPT.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		if exists, err := gitTagExists(args[0]); (err != nil) || !exists {
			println(fmt.Sprintf("Tag %s does not exist\n", args[0]))
			return
		}
		...
		if err != nil {
			println("Could not get git commits:", err)
			return
		}
		...
		if err != nil {
			println("Could not get remote URL:", err)
			return
		}
...
Repository: ` + string(url))).
		...
		if err != nil {
			println("Could not parse JSON object:", err)
			return
		}
		...
		if err := releaseCmd.Run(); err != nil {
			println(fmt.Sprintf("Could not create GitHub release. Title: %s, Notes: %s, Error: %s\n", title, notes, err))
			return
		}
	},
}
