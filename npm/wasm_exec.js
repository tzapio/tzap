...
(() => {
	...
	O_SYNC   = 010000
			...
			ftruncate(fd, length, callback) { fs.ftruncate(fd,length,callback); },
			lchown(path, uid, gid, callback) { fs.lchown(path,uid,gid,callback); },
	...
	if (!globalThis.process) {
		globalThis.process = {
			...
			chdir(directory) { return process.chdir(directory) },
		}
	}
